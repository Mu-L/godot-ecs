...
# 缓存特定组件组合的查询结果
var signature: Array[StringName] # 排序后的组件名列表
var results: Array = []          # 缓存的结果数组: Array[Dictionary]
var entity_indices: Dictionary = {} # EntityID -> index in results (用于快速查找移除)
var _world: ECSWorld

func _init(w: ECSWorld, sig: Array):
	_world = w
	signature.append_array(sig)
...
func _add(entity_id: int) -> void:
	if entity_indices.has(entity_id): return # 已经在缓存里了
	...
	entity_indices[entity_id] = results.size() - 1

# 从缓存移除实体 (Swap-back-and-pop O(1) 删除)
func _remove(entity_id: int) -> void:
	if not entity_indices.has(entity_id): return
	
	var idx = entity_indices[entity_id]
	...
	if idx != last_idx:
		...
		entity_indices[last_entity_id] = idx
...
func on_component_changed(entity_id: int, component_name: StringName, is_added: bool) -> void:
	...
	if not component_name in signature:
		return
		
	var in_cache = entity_indices.has(entity_id)
